...
def parsePage(ilt, html):
    try:
        ...
        for i in range(0,len(plt)+1):                                           
            ...
            ilt.append([title,price,local,pic_url])                          #在列表后新增一个元素
    except:
        print("")
...
def printGoodsList(ilt):
    tplt = "{:4}\t{:8}\t{:16}\t{:32}\t{:48}"
    print(tplt.format("序号","商品名称" ,"价格",  "地址", "图片"))
    count = 0
    for g in ilt:
        count = count + 1
...
df = pd.DataFrame(infoList,columns=['Titile','Price','Location','Pic_url'])  # 写入excel
